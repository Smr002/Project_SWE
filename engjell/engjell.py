...
class FRUIT:
    ...
    def draw_fruit(self):
        ...
        screen.blit(berry,fruit_rect) #e
        # draw the rectangle
...
class MAIN: #e
    ...
    def check_collision(self):
        if self.fruit.pos == self.snake.body[0]:
            #spawn fruit in random position
...
berry = pygame.image.load('Graphics/berry.png').convert_alpha() #e
berry = pygame.transform.scale(berry, (80, cell_size))

SCREEEN_UPDATE = pygame.USEREVENT       #e
pygame.time.set_timer(SCREEEN_UPDATE,150)
...
while True: #event loop
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
        if event.type == SCREEEN_UPDATE:    #e
            main_game.update()
